...
- 2026-09-12d (quiet re-measure attempted, per the 12c "quiet re-measure owed" caveat): the machine never got fully quiet (a 10-minute bounded wait held load at ~7, not the sub-4 target), but `stall_ms` still improved from 12c's load-186 695.6ms to 642.2ms p50 — and is **still FAIL against the 400ms budget**. `first_audio_ms`/`first_sentence_ms` (642.4ms), `barge_ms` (1.0ms), and cold-start `turn_worst_ms` (550.5ms) all PASS. The stage receipt (`server/turns.jsonl`) puts the whole breach in STT (590ms inside the server here vs. 143-164ms measured in isolation), which means the 12c diagnosis — pure load-186 artifact — was incomplete: STT inside this server is sensitive to ordinary background CPU contention, Kokoro's own synthesis threads included, not only to extreme load. Full table in `docs/SPEC.md` §9.1.
- 2026-09-12f: **streaming STT's turn-level `stall_ms` is still not measured, and now we know exactly why.** The `server/turn.py` `stall_sent` hook §9.2 was waiting on has landed, so this pass ran the live server both ways (`PET_TALK_STT_STREAM=0`/`1`) — `stall_ms` p50 came back statistically identical (223.1ms vs 194.3ms, load 9-13) because `qa/latency.py` and `qa/live_ws_turn.py` both hand the whole utterance to one `user.stop` message and never send a `user.chunk` frame, so `_stream_stop` (`server/ws.py`) never has a stream session to serve and every turn falls through to the whole-utterance path regardless of the flag (`server/turns.jsonl` shows `"path": "worker"` in every turn of both passes, never `"path": "stream"`). `PET_TALK_STT_STREAM` stays `0` — not because streaming failed, but because this pass couldn't honestly exercise it. Final transcript on the same clip was identical in all 6 captured turns either way. Full writeup in `docs/SPEC.md` §9.1 (2026-09-12f) and §9.2's follow-up note.
