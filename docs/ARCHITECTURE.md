...
| `LLM_PROVIDER`, `STT_PROVIDER`, `TTS_PROVIDER` | `haiku` (llm.py), `deepgram` if key set else `faster-whisper`, `smallest` if key set else `kokoro` | `settings.py`, `provider_factory.py` |
