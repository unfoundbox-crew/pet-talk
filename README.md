...
# 2. Kokoro TTS local daemon (:8088) — needed unless TTS_PROVIDER is something else
./serve.sh

# 3. pet-talk FastAPI duplex server (:8089)
...
| Variable | Purpose |
|---|---|
| `STT_PROVIDER` | `deepgram` \| `groq` \| `openai`/`openai-whisper` \| `whisperkit` \| `faster-whisper`/`local` \| `mlx` \| `sensevoice` \| `stub`. Defaults to `deepgram` if `DEEPGRAM_API_KEY` is set, else `faster-whisper`. |
| `LLM_PROVIDER` | `haiku`/`claude` \| `opencode`/`zen` \| `gemini`/`google`/`flash` \| `groq` \| `openai`/`gpt` \| `litellm`/`fleet`/`local` \| `stub`. Defaults to `groq` if `GROQ_API_KEY` is set, else `haiku` if `ANTHROPIC_API_KEY` is set, else `litellm`. |
| `TTS_PROVIDER` | `kokoro` \| `smallest`/`smallest-ai`/`waves` \| `elevenlabs` \| `deepgram` \| `stub`. Defaults to `smallest` if `SMALLEST_API_KEY` is set, else `kokoro`. |
